...
bool SIG2state = true; //NOTE: SIG2state is written as !SIG2state to keep the blue led "normally off"/quiet
bool SIG3state = false;

int alertLevel = 0;
//duplicates of the same i/o's
//d1 mini seems to crash when you use d4 :/
//d1 mini seems to crash when you use d6, booting to watchdog timer every ~10 seconds, while blue led is mainly off :/
int SIG2btn = D5;
int SIG3btn = D6; // switch, other side connected to ground
int SIG1read = 0;
int SIG2read = 0;
int SIG3read = 0;
...
int SIG2led = D4;     // LED on ~pin 2~ D4 is mobo led
...
void setup() {
  ...
  //ktb are these pullup methods NOT valid for the d1 mini? need to set with int??
...
void loop() {
  ...
  //poc code, probably best that the remote sends SIG2led high so the on-board led turns off ktb
  //SIG1read = digitalRead(SIG1btn);
  SIG2read = digitalRead(SIG2btn);
  SIG3read = digitalRead(SIG3btn);
